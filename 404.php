<!doctype php>
<php lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="x-ua-compatible" content="ie=edge" />
    <title>Kronik-X Health - Innover pour transformer la gestion des maladies chroniques en Afrique et dans les LMICs</title>
    <?php
			require __DIR__ . '/layouts/head.php';
		?>
</head>

<body>
    <!-- prettier-ignore -->
    <!--Preloader-->
    <?php
			require __DIR__ . '/layouts/preloader.php';
		?>
    <!--Preloader-end -->
    <!-- <div id="myOverlay" class="overlay search-popup">
        <div class="overlay-content">
            <span class="search-close closebtn" onclick="closeSearch()">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 24 24">
                    <path d="M 4.7070312 3.2929688 L 3.2929688 4.7070312 L 10.585938 12 L 3.2929688 19.292969 L 4.7070312 20.707031 L 12 13.414062 L 19.292969 20.707031 L 20.707031 19.292969 L 13.414062 12 L 20.707031 4.7070312 L 19.292969 3.2929688 L 12 10.585938 L 4.7070312 3.2929688 z"></path>
                </svg>
            </span>
            <form action="#">
                <input type="text" placeholder="Search.." name="search" />
                <button type="submit" class="btn btn-primary-500">Search</button>
            </form>
        </div>
    </div> -->

    <!-- Navbar -->
    <?php
                require __DIR__ . '/layouts/header.php';
            ?>

    <main>
        <!-- prettier-ignore -->
        <!--Home 11 Section 9 -->
        <section class="position-relative overflow-hidden box-latest-blog-3 box-latest-blog-12">
            <div class="container">
                <div class="text-center">
                    <img src="assets/imgs/pages/404/404_resized.webp" alt="kronik" class="mb-30" />
                    <h3 class="heading-inter-44 secondery-500 mb-4">Oops! Page Introuvable.</h3>
                    <a href="/" class="btn btn-primary-home btn-inter">Retour à la page d'accueil</a>
                </div>
            </div>
        </section>

    </main>
    <!-- prettier-ignore -->
    <!-- Footer 10 -->
    <div class="slide-footer-brand">
        <div class="slide-footer-brand-inner">
            <div class="slide-footer-brand-inner-2">
                <div class="container">
                    <div class="box-swiper">
                        <div class="swiper-container swiper-group-slide-text wow fadeInUp">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <div class="text-slide">
                                        <svg width="68" height="68" viewBox="0 0 68 68" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M65.5893 35.4981C65.5893 35.4981 62.8688 35.8606 57.7923 36.0941C52.7158 36.3276 45.2935 36.4326 35.9001 35.9182C34.8713 54.7051 32.2406 65.3841 32.2406 65.3841C32.2406 65.3841 31.8781 62.6636 31.6446 57.5871C31.4111 52.5106 31.306 45.0883 31.8204 35.6948C13.0336 34.6661 2.35458 32.0353 2.35458 32.0353C2.35458 32.0353 5.07508 31.6728 10.1516 31.4393C15.2281 31.2058 22.6504 31.1008 32.0438 31.6152C33.0726 12.8283 35.7033 2.14933 35.7033 2.14933C35.7033 2.14933 37.1522 13.0517 36.1235 31.8386C54.9103 32.8673 65.5893 35.4981 65.5893 35.4981Z" fill="white" />
                                            <path d="M36.2367 29.7959C42.803 21.9717 49.0342 20.2669 49.0342 20.2669C46.1496 24.7875 42.4665 28.7455 38.1648 31.9474M30.0055 31.5006C22.1813 24.9343 20.4766 18.7031 20.4766 18.7031C24.9971 21.5877 28.9551 25.2708 32.157 29.5725M31.7102 37.7318C25.1439 45.556 18.9128 47.2608 18.9128 47.2608C21.7973 42.7402 25.4805 38.7822 29.7821 35.5803M37.9414 36.0271C45.7657 42.5934 47.4704 48.8246 47.4704 48.8246C42.9498 45.94 38.9918 42.2569 35.7899 37.9552" fill="white" />
                                            <path d="M33.6895 38.869C36.5059 39.0233 38.9141 36.8651 39.0683 34.0487C39.2226 31.2323 37.0644 28.8241 34.248 28.6699C31.4316 28.5157 29.0234 30.6738 28.8692 33.4902C28.715 36.3066 30.8731 38.7148 33.6895 38.869Z" fill="white" />
                                        </svg>
                                        Corporate
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="text-slide">
                                        <svg width="68" height="68" viewBox="0 0 68 68" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M65.5893 35.4981C65.5893 35.4981 62.8688 35.8606 57.7923 36.0941C52.7158 36.3276 45.2935 36.4326 35.9001 35.9182C34.8713 54.7051 32.2406 65.3841 32.2406 65.3841C32.2406 65.3841 31.8781 62.6636 31.6446 57.5871C31.4111 52.5106 31.306 45.0883 31.8204 35.6948C13.0336 34.6661 2.35458 32.0353 2.35458 32.0353C2.35458 32.0353 5.07508 31.6728 10.1516 31.4393C15.2281 31.2058 22.6504 31.1008 32.0438 31.6152C33.0726 12.8283 35.7033 2.14933 35.7033 2.14933C35.7033 2.14933 37.1522 13.0517 36.1235 31.8386C54.9103 32.8673 65.5893 35.4981 65.5893 35.4981Z" fill="white" />
                                            <path d="M36.2367 29.7959C42.803 21.9717 49.0342 20.2669 49.0342 20.2669C46.1496 24.7875 42.4665 28.7455 38.1648 31.9474M30.0055 31.5006C22.1813 24.9343 20.4766 18.7031 20.4766 18.7031C24.9971 21.5877 28.9551 25.2708 32.157 29.5725M31.7102 37.7318C25.1439 45.556 18.9128 47.2608 18.9128 47.2608C21.7973 42.7402 25.4805 38.7822 29.7821 35.5803M37.9414 36.0271C45.7657 42.5934 47.4704 48.8246 47.4704 48.8246C42.9498 45.94 38.9918 42.2569 35.7899 37.9552" fill="white" />
                                            <path d="M33.6895 38.869C36.5059 39.0233 38.9141 36.8651 39.0683 34.0487C39.2226 31.2323 37.0644 28.8241 34.248 28.6699C31.4316 28.5157 29.0234 30.6738 28.8692 33.4902C28.715 36.3066 30.8731 38.7148 33.6895 38.869Z" fill="white" />
                                        </svg>
                                        Business
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="text-slide">
                                        <svg width="68" height="68" viewBox="0 0 68 68" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M65.5893 35.4981C65.5893 35.4981 62.8688 35.8606 57.7923 36.0941C52.7158 36.3276 45.2935 36.4326 35.9001 35.9182C34.8713 54.7051 32.2406 65.3841 32.2406 65.3841C32.2406 65.3841 31.8781 62.6636 31.6446 57.5871C31.4111 52.5106 31.306 45.0883 31.8204 35.6948C13.0336 34.6661 2.35458 32.0353 2.35458 32.0353C2.35458 32.0353 5.07508 31.6728 10.1516 31.4393C15.2281 31.2058 22.6504 31.1008 32.0438 31.6152C33.0726 12.8283 35.7033 2.14933 35.7033 2.14933C35.7033 2.14933 37.1522 13.0517 36.1235 31.8386C54.9103 32.8673 65.5893 35.4981 65.5893 35.4981Z" fill="white" />
                                            <path d="M36.2367 29.7959C42.803 21.9717 49.0342 20.2669 49.0342 20.2669C46.1496 24.7875 42.4665 28.7455 38.1648 31.9474M30.0055 31.5006C22.1813 24.9343 20.4766 18.7031 20.4766 18.7031C24.9971 21.5877 28.9551 25.2708 32.157 29.5725M31.7102 37.7318C25.1439 45.556 18.9128 47.2608 18.9128 47.2608C21.7973 42.7402 25.4805 38.7822 29.7821 35.5803M37.9414 36.0271C45.7657 42.5934 47.4704 48.8246 47.4704 48.8246C42.9498 45.94 38.9918 42.2569 35.7899 37.9552" fill="white" />
                                            <path d="M33.6895 38.869C36.5059 39.0233 38.9141 36.8651 39.0683 34.0487C39.2226 31.2323 37.0644 28.8241 34.248 28.6699C31.4316 28.5157 29.0234 30.6738 28.8692 33.4902C28.715 36.3066 30.8731 38.7148 33.6895 38.869Z" fill="white" />
                                        </svg>
                                        Consulting
                                    </div>
                                </div>
                                <div class="swiper-slide">
                                    <div class="text-slide">
                                        <svg width="68" height="68" viewBox="0 0 68 68" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M65.5893 35.4981C65.5893 35.4981 62.8688 35.8606 57.7923 36.0941C52.7158 36.3276 45.2935 36.4326 35.9001 35.9182C34.8713 54.7051 32.2406 65.3841 32.2406 65.3841C32.2406 65.3841 31.8781 62.6636 31.6446 57.5871C31.4111 52.5106 31.306 45.0883 31.8204 35.6948C13.0336 34.6661 2.35458 32.0353 2.35458 32.0353C2.35458 32.0353 5.07508 31.6728 10.1516 31.4393C15.2281 31.2058 22.6504 31.1008 32.0438 31.6152C33.0726 12.8283 35.7033 2.14933 35.7033 2.14933C35.7033 2.14933 37.1522 13.0517 36.1235 31.8386C54.9103 32.8673 65.5893 35.4981 65.5893 35.4981Z" fill="white" />
                                            <path d="M36.2367 29.7959C42.803 21.9717 49.0342 20.2669 49.0342 20.2669C46.1496 24.7875 42.4665 28.7455 38.1648 31.9474M30.0055 31.5006C22.1813 24.9343 20.4766 18.7031 20.4766 18.7031C24.9971 21.5877 28.9551 25.2708 32.157 29.5725M31.7102 37.7318C25.1439 45.556 18.9128 47.2608 18.9128 47.2608C21.7973 42.7402 25.4805 38.7822 29.7821 35.5803M37.9414 36.0271C45.7657 42.5934 47.4704 48.8246 47.4704 48.8246C42.9498 45.94 38.9918 42.2569 35.7899 37.9552" fill="white" />
                                            <path d="M33.6895 38.869C36.5059 39.0233 38.9141 36.8651 39.0683 34.0487C39.2226 31.2323 37.0644 28.8241 34.248 28.6699C31.4316 28.5157 29.0234 30.6738 28.8692 33.4902C28.715 36.3066 30.8731 38.7148 33.6895 38.869Z" fill="white" />
                                        </svg>
                                        Creative
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <footer class="footer-style-10 footer-style-red">
        <div class="container-fluid ft-bgr section-footer">
            <div class="container">
                <div class="top-footer">
                    <div class="top-footer-left">
                        <h4 class="heading-ag-3xl color-white">Have an idea lets <br />work together ?</h4>
                    </div>
                    <div class="top-footer-right">
                        <div class="form-newsletter">
                            <form action="#">
                                <input type="text" class="form-control" placeholder="Email Address" />
                                <input type="submit" class="btn btn-newsletter" value="" />
                            </form>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-3 col-md-6">
                        <a class="navbar-brand pe-4" href="index.html"><img src="assets/imgs/template/logo-footer-4.svg" alt="" /></a>
                        <p class="paragraph-base color-white pt-4 pb-2">we denounce with righteous indignation and dislike men who are so beguiled demoralized by the charms</p>
                        <div class="box-social-profile pt-2">
                            <div class="list-socials">
                                <a href="#" class="share">
                                    <svg width="9" height="15" viewBox="0 0 9 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M7.62891 8.14062H5.57812V14.2656H2.84375V8.14062H0.601562V5.625H2.84375V3.68359C2.84375 1.49609 4.15625 0.265625 6.15234 0.265625C7.10938 0.265625 8.12109 0.457031 8.12109 0.457031V2.61719H7C5.90625 2.61719 5.57812 3.27344 5.57812 3.98438V5.625H8.01172L7.62891 8.14062Z" fill="" />
                                    </svg>
                                </a>
                                <a href="#" class="share">
                                    <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M10.0596 6.86085L15.8879 0.0859375H14.5068L9.44607 5.96849L5.40411 0.0859375H0.742188L6.85442 8.98141L0.742188 16.0859H2.12338L7.4676 9.87376L11.7362 16.0859H16.3981L10.0593 6.86085H10.0596ZM8.16787 9.05978L7.54857 8.17399L2.62104 1.12568H4.74248L8.71905 6.81389L9.33834 7.69968L14.5074 15.0935H12.386L8.16787 9.06012V9.05978Z" fill="" />
                                    </svg>
                                </a>
                                <a href="#" class="share">
                                    <svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M11.375 0.140625C11.8398 0.140625 12.25 0.550781 12.25 1.04297V11.5156C12.25 12.0078 11.8398 12.3906 11.375 12.3906H0.847656C0.382812 12.3906 0 12.0078 0 11.5156V1.04297C0 0.550781 0.382812 0.140625 0.847656 0.140625H11.375ZM3.69141 10.6406V4.81641H1.88672V10.6406H3.69141ZM2.78906 3.99609C3.36328 3.99609 3.82812 3.53125 3.82812 2.95703C3.82812 2.38281 3.36328 1.89062 2.78906 1.89062C2.1875 1.89062 1.72266 2.38281 1.72266 2.95703C1.72266 3.53125 2.1875 3.99609 2.78906 3.99609ZM10.5 10.6406V7.44141C10.5 5.88281 10.1445 4.65234 8.3125 4.65234C7.4375 4.65234 6.83594 5.14453 6.58984 5.60938H6.5625V4.81641H4.83984V10.6406H6.64453V7.76953C6.64453 7.00391 6.78125 6.26562 7.73828 6.26562C8.66797 6.26562 8.66797 7.14062 8.66797 7.79688V10.6406H10.5Z" fill="" />
                                    </svg>
                                </a>
                                <a href="#" class="share">
                                    <svg width="11" height="15" viewBox="0 0 11 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5.57812 0.457031C8.14844 0.457031 10.5 2.23438 10.5 4.94141C10.5 7.48438 9.1875 10.3281 6.28906 10.3281C5.57812 10.3281 4.73047 9.97266 4.375 9.34375C3.77344 11.8047 3.80078 12.1875 2.43359 14.0742C2.29688 14.1289 2.32422 14.1289 2.24219 14.0195C2.1875 13.5 2.13281 13.0078 2.13281 12.4883C2.13281 10.8203 2.89844 8.38672 3.28125 6.77344C3.0625 6.33594 3.00781 5.84375 3.00781 5.37891C3.00781 3.19141 5.57812 2.86328 5.57812 4.66797C5.57812 5.73438 4.83984 6.74609 4.83984 7.78516C4.83984 8.46875 5.44141 8.96094 6.125 8.96094C8.01172 8.96094 8.58594 6.25391 8.58594 4.80469C8.58594 2.86328 7.21875 1.79688 5.33203 1.79688C3.17188 1.79688 1.50391 3.35547 1.50391 5.54297C1.50391 6.60938 2.16016 7.15625 2.16016 7.40234C2.16016 7.62109 1.99609 8.35938 1.72266 8.35938C1.06641 8.35938 0 7.26562 0 5.35156C0 2.31641 2.76172 0.457031 5.57812 0.457031Z" fill="" />
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-2 col-md-6">
                        <h3 class="text-footer pb-1 mb-4">Services</h3>
                        <div class="d-flex flex-column align-items-start">
                            <a class="hover-effect paragraph-base grey-100 pt-1" href="#">
                                <svg width="8" height="11" viewBox="0 0 8 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.75 11C1.50391 11 1.28516 10.918 1.12109 10.7539C0.765625 10.4258 0.765625 9.85156 1.12109 9.52344L4.86719 5.75L1.12109 2.00391C0.765625 1.67578 0.765625 1.10156 1.12109 0.773438C1.44922 0.417969 2.02344 0.417969 2.35156 0.773438L6.72656 5.14844C7.08203 5.47656 7.08203 6.05078 6.72656 6.37891L2.35156 10.7539C2.1875 10.918 1.96875 11 1.75 11Z" fill="" />
                                </svg>
                                Ui Design
                            </a>
                            <a class="hover-effect paragraph-base grey-100 pt-2" href="#">
                                <svg width="8" height="11" viewBox="0 0 8 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.75 11C1.50391 11 1.28516 10.918 1.12109 10.7539C0.765625 10.4258 0.765625 9.85156 1.12109 9.52344L4.86719 5.75L1.12109 2.00391C0.765625 1.67578 0.765625 1.10156 1.12109 0.773438C1.44922 0.417969 2.02344 0.417969 2.35156 0.773438L6.72656 5.14844C7.08203 5.47656 7.08203 6.05078 6.72656 6.37891L2.35156 10.7539C2.1875 10.918 1.96875 11 1.75 11Z" fill="" />
                                </svg>
                                Ux Design
                            </a>
                            <a class="hover-effect paragraph-base grey-100 pt-2" href="#">
                                <svg width="8" height="11" viewBox="0 0 8 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.75 11C1.50391 11 1.28516 10.918 1.12109 10.7539C0.765625 10.4258 0.765625 9.85156 1.12109 9.52344L4.86719 5.75L1.12109 2.00391C0.765625 1.67578 0.765625 1.10156 1.12109 0.773438C1.44922 0.417969 2.02344 0.417969 2.35156 0.773438L6.72656 5.14844C7.08203 5.47656 7.08203 6.05078 6.72656 6.37891L2.35156 10.7539C2.1875 10.918 1.96875 11 1.75 11Z" fill="" />
                                </svg>
                                Digital Marketing
                            </a>
                            <a class="hover-effect paragraph-base grey-100 pt-2" href="#">
                                <svg width="8" height="11" viewBox="0 0 8 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.75 11C1.50391 11 1.28516 10.918 1.12109 10.7539C0.765625 10.4258 0.765625 9.85156 1.12109 9.52344L4.86719 5.75L1.12109 2.00391C0.765625 1.67578 0.765625 1.10156 1.12109 0.773438C1.44922 0.417969 2.02344 0.417969 2.35156 0.773438L6.72656 5.14844C7.08203 5.47656 7.08203 6.05078 6.72656 6.37891L2.35156 10.7539C2.1875 10.918 1.96875 11 1.75 11Z" fill="" />
                                </svg>
                                Video Editing
                            </a>
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                        <h3 class="text-footer pb-1 mb-4">Contact</h3>
                        <div class="pt-0">
                            <p class="text-inline">
                                <a href="mailto:dolores.chambers@example.com">
                                    <svg class="me-2" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12.6875 1.5C13.3984 1.5 14 2.10156 14 2.8125C14 3.25 13.7812 3.63281 13.4531 3.87891L7.51953 8.33594C7.19141 8.58203 6.78125 8.58203 6.45312 8.33594L0.519531 3.87891C0.191406 3.63281 0 3.25 0 2.8125C0 2.10156 0.574219 1.5 1.3125 1.5H12.6875ZM5.93359 9.04688C6.5625 9.51172 7.41016 9.51172 8.03906 9.04688L14 4.5625V10.25C14 11.2344 13.207 12 12.25 12H1.75C0.765625 12 0 11.2344 0 10.25V4.5625L5.93359 9.04688Z" fill="" />
                                    </svg>
                                    <span class="color-white-70">dolores.chambers@example.com</span>
                                </a>
                            </p>
                            <p class="text-inline">
                                <svg class="me-2" width="11" height="15" viewBox="0 0 11 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.59375 14.4219C3.17188 12.6445 0 8.40625 0 6C0 3.10156 2.32422 0.75 5.25 0.75C8.14844 0.75 10.5 3.10156 10.5 6C10.5 8.40625 7.30078 12.6445 5.87891 14.4219C5.55078 14.832 4.92188 14.832 4.59375 14.4219ZM5.25 7.75C6.20703 7.75 7 6.98438 7 6C7 5.04297 6.20703 4.25 5.25 4.25C4.26562 4.25 3.5 5.04297 3.5 6C3.5 6.98438 4.26562 7.75 5.25 7.75Z" fill="" />
                                </svg>
                                <span class="color-white-70">1901 Thornridge Cir. Shiloh, Hawaii 81063</span>
                            </p>
                            <p class="text-inline">
                                <svg class="me-2" width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.6199 10.759L11.6661 8.80522C10.9684 8.10746 9.78218 8.38659 9.50307 9.29366C9.29374 9.92167 8.59598 10.2706 7.96799 10.131C6.57246 9.78209 4.68849 7.9679 4.33961 6.5026C4.13028 5.87458 4.54894 5.17682 5.17693 4.96751C6.08402 4.68841 6.36313 3.50221 5.66536 2.80444L3.71162 0.850702C3.15341 0.362266 2.31609 0.362266 1.82766 0.850702L0.501902 2.17646C-0.823851 3.57198 0.641455 7.27014 3.92095 10.5496C7.20045 13.8291 10.8986 15.3642 12.2941 13.9687L13.6199 12.6429C14.1083 12.0847 14.1083 11.2474 13.6199 10.759Z" fill="" />
                                </svg>
                                <span class="color-white-70">+123 888 9999</span>
                            </p>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <h3 class="text-footer pb-1 mb-4">Recent News</h3>
                        <div class="d-flex flex-column align-items-start">
                            <ul class="recent-news-list">
                                <li>
                                    <div class="news-image">
                                        <a href="#"><img src="assets/imgs/pages/home13/news2.png" alt="Vatech" /></a>
                                    </div>
                                    <div class="news-info">
                                        <div class="news-postdate">
                                            <svg width="11" height="13" viewBox="0 0 11 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M3.5625 2H6.9375V1.0625C6.9375 0.757812 7.17188 0.5 7.5 0.5C7.80469 0.5 8.0625 0.757812 8.0625 1.0625V2H9C9.82031 2 10.5 2.67969 10.5 3.5V11C10.5 11.8438 9.82031 12.5 9 12.5H1.5C0.65625 12.5 0 11.8438 0 11V3.5C0 2.67969 0.65625 2 1.5 2H2.4375V1.0625C2.4375 0.757812 2.67188 0.5 3 0.5C3.30469 0.5 3.5625 0.757812 3.5625 1.0625V2ZM1.125 6.3125H3V5H1.125V6.3125ZM1.125 7.4375V8.9375H3V7.4375H1.125ZM4.125 7.4375V8.9375H6.375V7.4375H4.125ZM7.5 7.4375V8.9375H9.375V7.4375H7.5ZM9.375 5H7.5V6.3125H9.375V5ZM9.375 10.0625H7.5V11.375H9C9.1875 11.375 9.375 11.2109 9.375 11V10.0625ZM6.375 10.0625H4.125V11.375H6.375V10.0625ZM3 10.0625H1.125V11C1.125 11.2109 1.28906 11.375 1.5 11.375H3V10.0625ZM6.375 5H4.125V6.3125H6.375V5Z" fill="" />
                                            </svg>
                                            <span class="paragraph-rubik-r color-white-70">17 oct 23</span>
                                        </div>
                                        <a href="#" class="paragraph-rubik-r news-link">Market research helps businesses understand</a>
                                    </div>
                                </li>
                                <li>
                                    <div class="news-image">
                                        <a href="#"><img src="assets/imgs/pages/home13/news3.png" alt="Vatech" /></a>
                                    </div>
                                    <div class="news-info">
                                        <div class="news-postdate">
                                            <svg width="11" height="13" viewBox="0 0 11 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M3.5625 2H6.9375V1.0625C6.9375 0.757812 7.17188 0.5 7.5 0.5C7.80469 0.5 8.0625 0.757812 8.0625 1.0625V2H9C9.82031 2 10.5 2.67969 10.5 3.5V11C10.5 11.8438 9.82031 12.5 9 12.5H1.5C0.65625 12.5 0 11.8438 0 11V3.5C0 2.67969 0.65625 2 1.5 2H2.4375V1.0625C2.4375 0.757812 2.67188 0.5 3 0.5C3.30469 0.5 3.5625 0.757812 3.5625 1.0625V2ZM1.125 6.3125H3V5H1.125V6.3125ZM1.125 7.4375V8.9375H3V7.4375H1.125ZM4.125 7.4375V8.9375H6.375V7.4375H4.125ZM7.5 7.4375V8.9375H9.375V7.4375H7.5ZM9.375 5H7.5V6.3125H9.375V5ZM9.375 10.0625H7.5V11.375H9C9.1875 11.375 9.375 11.2109 9.375 11V10.0625ZM6.375 10.0625H4.125V11.375H6.375V10.0625ZM3 10.0625H1.125V11C1.125 11.2109 1.28906 11.375 1.5 11.375H3V10.0625ZM6.375 5H4.125V6.3125H6.375V5Z" fill="" />
                                            </svg>
                                            <span class="paragraph-rubik-r color-white-70">17 oct 23</span>
                                        </div>
                                        <a href="#" class="paragraph-rubik-r news-link">Common types of business structure</a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="container-fluid footer-1 px-0">
            <div class="footer">
                <div class="container py-4">
                    <div class="footer-bottom">
                        <div class="row paragraph-base">
                            <div class="col-sm-6 col-12 text-center text-md-start">
                                <div class="paragraph-rubik-r color-white-70">Copyright © Vatech 2024. Alright Reserved.</div>
                            </div>
                            <div class="col-sm-6 col-12 d-flex justify-content-center justify-content-md-end">
                                <ul class="menu-footer">
                                    <li><a href="#">Trams & Condition</a></li>
                                    <li><a href="#">Privacy Policy</a></li>
                                    <li><a href="#">Contact Us</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </footer>

    <!-- Scroll top -->
    <div class="btn-scroll-top">
        <svg class="progress-square svg-content" width="100%" height="100%" viewBox="0 0 40 40">
            <path d="M8 1H32C35.866 1 39 4.13401 39 8V32C39 35.866 35.866 39 32 39H8C4.13401 39 1 35.866 1 32V8C1 4.13401 4.13401 1 8 1Z" />
        </svg>
    </div>

    <!-- Libs JS -->
    <script src="assets/js/vendors/jquery-3.7.1.min.js"></script>
    <script src="assets/js/vendors/bootstrap.bundle.min.js"></script>
    <script src="assets/js/vendors/masonry.pkgd.min.js" integrity="sha384-GNFwBvfVxBkLMJpYMOABq3c+d3KnQxudP/mGPkzpZSTYykLBNsZEnG2D9G/X/+7D" crossorigin="anonymous" async></script>
    <script src="assets/js/vendors/swiper-bundle.min.js"></script>
    <script src="assets/js/vendors/aos.js"></script>
    <script src="assets/js/vendors/wow.min.js"></script>
    <script src="assets/js/vendors/jquery.appear.js"></script>
    <script src="assets/js/vendors/jquery.odometer.min.js"></script>
    <script src="assets/js/vendors/headhesive.min.js"></script>
    <script src="assets/js/vendors/smart-stick-nav.js"></script>
    <script src="assets/js/vendors/jquery.magnific-popup.min.js"></script>
    <script src="assets/js/vendors/gsap.min.js"></script>
    <script src="assets/js/vendors/ScrollToPlugin.min.js"></script>
    <script src="assets/js/vendors/ScrollTrigger.min.js"></script>
    <!-- Theme JS -->
    <script src="assets/js/gsap-custom.js"></script>
    <script src="assets/js/main.js"></script>

</body>

</html>